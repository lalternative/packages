...
func TestNewSignerNeedsAURLAnIssuerAndAKey(t *testing.T) {
	for _, tc := range []struct {
		...
		{"no key", SignerConfig{PublicURL: "https://audio.example", Issuer: "lalter"}},
		{"no url", SignerConfig{Issuer: "lalter", Key: "secret"}},
		{"no issuer", SignerConfig{PublicURL: "https://audio.example", Key: "secret"}},
...
// The URL is what tornade will check, so it has to verify against the same
// secret: this is the contract between the two services.
func TestSignedURLVerifies(t *testing.T) {
	...
	if err := v.Verify(parsed.Query(), "chat-message", "msg-42", "bonjour"); err != nil {
		t.Fatalf("tornade would refuse the URL we handed out: %v", err)
	}
}
